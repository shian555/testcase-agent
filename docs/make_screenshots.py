...
with sync_playwright() as p:
    ...
    page.wait_for_timeout(14000)  # 图表/表格经 websocket 异步水合，需等足
    ...
    # 图5：测试报告（KPI 带 + 结论横幅 + 结果分布图表 + 问题清单）
    page.goto(f"{BASE}/reports", wait_until="networkidle")
    page.wait_for_timeout(14000)
    page.screenshot(path=f"{OUT}/demo_report.png")

    # 图6：缺陷管理（KPI 带 + 筛选器 + 状态/严重度色块表格）
    page.goto(f"{BASE}/defects", wait_until="networkidle")
    page.wait_for_timeout(9000)
    page.screenshot(path=f"{OUT}/demo_defects.png")

    browser.close()

print("已输出 demo_home / demo_result / demo_cases / demo_execution / demo_report / demo_defects")
