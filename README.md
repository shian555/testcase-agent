...
| **用例库** | 资产概览指标条（总数 / P0 / AI / 手工 / 停用）；关键字 + 方法 + 优先级 + 来源 + 状态组合筛选；多选圈选 → 直接建测试单；维护区三列收拢（新建手工用例 / **Excel 批量导入存量用例** / 单条详情与编辑）；停用 / 删除 |
| **测试执行** | 从用例库圈选创建测试单（名称 / 环境 / **执行人** / 备注）；执行面板：进度条、结果 pill 统计、**按结果筛选**、逐条结果下拉 + 失败备注、**未执行一键批量置为通过**；未执行清零才能"标记完成"；Excel 执行报表 |
