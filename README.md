...
| 页面 | 能力 |
|---|---|
| **工作台** | 用例总数 / AI 占比 / 测试单 / 待处理缺陷 / 最近通过率 5 张 KPI 卡；2×2 图表网格：方法分布环形图、优先级×来源堆叠柱、**模块通过率质量图**、通过率趋势折线；最近测试单表（执行人 / 进度条）；空库一键填充演示数据（电商场景：用户注册 / 购物车两模块手工用例 + 登录模块 AI 用例，3 张测试单覆盖 进行中 / 回归完成 / 冒烟完成，5 条缺陷覆盖全部状态） |
| **用例生成** | 粘贴 PRD → AnalyzerAgent → GeneratorAgent → ReviewerAgent 全程可视化，5 个页签（测试点 / 用例 / 评审详情 / **采纳入库** / 导出 CSV·MD·Excel）；入库时可逐行标注**用例类型**（功能 / UI / 接口 / 性能 / 安全 / 兼容） |
| **用例库** | 资产概览指标条（总数 / P0 / AI / 手工 / 停用）+ 类型分布 chips；关键字 + 方法 + **类型** + 优先级 + 来源 + 状态组合筛选；多选圈选 → 直接建测试单；维护区三列收拢（新建手工用例 / **Excel 批量导入存量用例** / 单条详情与编辑）；停用 / 删除 |
| **测试执行** | 从用例库圈选创建测试单（名称 / 环境 / **执行人** / 备注，圈选支持按类型 / 方法 / 优先级过滤）；执行面板：进度条、结果 pill 统计、**按结果筛选**、逐条结果下拉 + 失败备注、**未执行一键批量置为通过**；未执行清零才能"标记完成"；Excel 执行报表 |
| **测试报告** | 6 项 KPI 含**通过率环比上一单** + 规则化发布建议（P0 失败 ⛔不建议发布）+ 结果分布 / 优先级通过率 / 模块×结果三张图 + 失败问题清单**一键登记缺陷** + **用例追溯矩阵**（用例 → 结果 → 关联缺陷）+ Excel（含追溯矩阵 sheet）/ Markdown 导出 |
| **缺陷管理** | 缺陷总数 / 打开 / 修复中 / 已解决 / 已关闭 5 项统计；关键字·状态·严重程度·模块筛选；状态流转（打开 → 修复中 → 已解决 → 已关闭）+ **处理人指派**；从失败用例或手工创建，与用例 / 测试单双向关联 |
...
## 测试方法论与用例类型（methods.py）

生成流程显式注入四种核心方法：**等价类 / 边界值 / 场景法 / 错误猜测**，每条用例都标注 `method`，可追溯到测试点；
平台另含 **判定表 / 正交试验 / 状态迁移** 三种补充方法，供手工设计与用例标注使用。

每条用例同时标注 **用例类型**：`功能 / UI / 接口 / 性能 / 安全 / 兼容`（与设计方法正交——
接口用例同样可以用等价类设计），支持筛选、统计与 Excel 导出；演示数据覆盖全部六种类型。
